...
//card component
function Card({data, count, setCount, index}){
  //state handling add to cart button
  const [show, setShow] = useState(true);
  const isSale = index === 1 || index === 2 || index === 4 || index === 6;
  ...
  return (
    <>
      <div className="card">
        {isSale && <div className="badge bg-dark text-white position-absolute sale-badge">Sale</div>}
    ...
    <>
      <div className="container text-center main">
        {products.map((value, index) => (
          <Card data={value} key={index} count={count} setCount={setCount} index={index}/>
        ))}
      </div>
    </>
  )
